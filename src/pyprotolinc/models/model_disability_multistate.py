...
class MultiStateDisabilityModel(Model):
    ...
    MODEL_NAME = "MultiStateDisability"
...
class ProjectionState(ModelState):
    ...
    def __init__(self, model, num_timesteps, portfolio, rows_for_state_recorder=None):
        ...
        # self._run_config.years_to_simulate * 12 * self._run_config.steps_per_month
        ...
        # self.state_matrix = np.zeros((self.num_records, self.numcls))
        # self.state_matrix[np.arange(self.num_records), self._portfolio.initialcls] = 1
        ...
        self.prob_state_active = np.zeros((self.num_records, 2))
        self.prob_state_dis1 = np.zeros((self.num_records, 2))
        self.prob_state_dis2 = np.zeros((self.num_records, 2))
        self.prob_state_death = np.zeros((self.num_records, 2))
        self.prob_state_lapsed = np.zeros((self.num_records, 2))

        # initialize
        self.prob_state_active[:, 1] = self._portfolio.initial_states == MultiStateDisabilityStates.ACTIVE
        self.prob_state_dis1[:, 1] = self._portfolio.initial_states == MultiStateDisabilityStates.DIS1
        self.prob_state_dis2[:, 1] = self._portfolio.initial_states == MultiStateDisabilityStates.DIS2
        self.prob_state_death[:, 1] = self._portfolio.initial_states == MultiStateDisabilityStates.DEATH
        self.prob_state_lapsed[:, 1] = self._portfolio.initial_states == MultiStateDisabilityStates.LAPSED
        ...
        if self.rows_for_state_recorder:
            ...
            self.state_recorder = np.zeros((1 + self.num_timesteps, len(self.state_recorder_indexes), self.numcls))
            self.state_recorder[self.step, :, 0] = self.prob_state_active[self.state_recorder_indexes, 1]
            self.state_recorder[self.step, :, 1] = self.prob_state_dis1[self.state_recorder_indexes, 1]
            self.state_recorder[self.step, :, 2] = self.prob_state_dis2[self.state_recorder_indexes, 1]
            self.state_recorder[self.step, :, 3] = self.prob_state_death[self.state_recorder_indexes, 1]
    ...
    def update_state_matrix(self, transition_ass_timestep):
        ...
        # dis1 -> *
        prob_mvm_dis1_death = transition_ass_timestep[:, MultiStateDisabilityStates.DIS1, MultiStateDisabilityStates.DEATH] * prob_state_dis1
        ...
        self.prob_state_dis1[:, 0] += prob_mvm_active_dis1 + prob_mvm_dis2_dis1 \
            - prob_mvm_dis1_death - prob_mvm_dis1_active - prob_mvm_dis1_dis2
        ...
        self.prob_state_death[:, 0] += prob_mvm_active_death + prob_mvm_dis1_death + prob_mvm_dis2_death
        ...
        if self.rows_for_state_recorder:
            ...
            self.state_recorder[self.step, :, 1] = self.prob_state_dis1[self.state_recorder_indexes, :].sum(axis=1)
            self.state_recorder[self.step, :, 2] = self.prob_state_dis2[self.state_recorder_indexes, :].sum(axis=1)
            self.state_recorder[self.step, :, 3] = self.prob_state_death[self.state_recorder_indexes, :].sum(axis=1)
